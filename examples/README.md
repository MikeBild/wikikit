...
| [`agent-hooks/`](agent-hooks)                                          | Product-neutral SessionStart/UserPromptSubmit/SessionEnd examples (`.sh` + `.ps1` pairs) — load a compact briefing, select per-prompt context, capture what the user taught. Install with `curl -fsSL <your-wikikit>/install.sh \| sh`. Setup: [docs/coding-agent-integration.md](../docs/coding-agent-integration.md). |
