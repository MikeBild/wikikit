...
const EN = {
  ...
  // §15.1 — the page heading. „Audit" is the MENU entry (nav.audit) and stays
  // that; the heading is „Audit-Trail" and the paragraph forbids „Protokoll",
  ...
  // THE OUTCOME WORDS STAY, AND NOT AS DECORATION.
  //
  // The rewritten audit page speaks §15.2's vocabulary (`audit.op.*`,
  // `audit.result.*`), not these. They remain because scripts/konvention-check.mjs
  // DERIVES §8.5's „finished work" vocabulary from every `audit.outcome.` key —
  // the words that may not appear on the decisions queue — and deleting them
  // would shrink that list silently, which is the failure mode §15's own guard
  // was written for. The check now asserts the size of that derivation too.
  ...
  'audit.column.transport': 'Channel',
  // §15.2's „Vorgang" — one German name per action the chain can carry.
  'audit.op.proposalApproved': 'Change approved',
  ...
  // The fallback behind „Verursacher": how the actor was established.
  'audit.actor.identity': 'Person',
  'audit.actor.apiKey': 'API key',
  'audit.actor.operatorSession': 'Operator session',
  'audit.actor.system': 'System',
  'audit.actor.anonymous': 'Unauthenticated',
  'audit.all': 'All',
  'audit.filter.operation': 'Operation',
  'audit.filter.result': 'Outcome',
  'audit.count': '{count} of {total} events',
  'audit.countCapped': '{count} of more than {total} events',
  'audit.loadMore': 'Load more',
  'audit.details': 'Details',
  'audit.fold': 'Close',
  'audit.detail.sequence': 'Sequence number',
  'audit.detail.prevHash': 'Hash of the previous event',
  'audit.detail.hash': 'Hash of this event',
  'audit.detail.before': 'Before',
  'audit.detail.after': 'After',
  'audit.detail.metadata': 'Context',
  'audit.footnote':
    'This chain records decisions about knowledge changes — including the ones that were refused or failed — and nothing else. Agent runs stand in the inbox, page revisions in the history of the page itself, guideline versions with the guidelines. Reads and searches, sign-ins, changes to keys and permissions and webhook deliveries are not recorded at all. A cause is named only where the record carries one.',
  'audit.empty.title': 'Nothing recorded yet',
  'audit.empty.description': 'As soon as a knowledge change has been decided, it stands here.',
...
const DE: Record<TranslationKey, string> = {
  ...
  'audit.column.transport': 'Kanal',
  // §15.2 „Vorgang" — je ein deutscher Name für eine Handlung der Kette.
  'audit.op.proposalApproved': 'Änderung freigegeben',
  ...
  'audit.result.denied': 'Verweigert',
  'audit.result.error': 'Fehlgeschlagen',
  'audit.result.cancelled': 'Abgebrochen',
  // Rückfallebene hinter „Verursacher": wie der Akteur festgestellt wurde.
  'audit.actor.identity': 'Person',
  'audit.actor.apiKey': 'API-Schlüssel',
  'audit.actor.operatorSession': 'Bediensitzung',
  'audit.actor.system': 'System',
  'audit.actor.anonymous': 'Ohne Anmeldung',
  'audit.all': 'Alle',
  'audit.filter.operation': 'Vorgang',
  'audit.filter.result': 'Ergebnis',
  'audit.count': '{count} von {total} Ereignissen',
  'audit.countCapped': '{count} von mehr als {total} Ereignissen',
  'audit.loadMore': 'Mehr laden',
  'audit.details': 'Details',
  'audit.fold': 'Schließen',
  'audit.detail.sequence': 'Laufende Nummer',
  'audit.detail.prevHash': 'Hash des vorherigen Ereignisses',
  'audit.detail.hash': 'Hash dieses Ereignisses',
  'audit.detail.before': 'Vorher',
  'audit.detail.after': 'Nachher',
  'audit.detail.metadata': 'Zusammenhang',
  'audit.footnote':
    'Diese Kette hält Entscheidungen über Wissensänderungen fest — auch die verweigerten und die fehlgeschlagenen — und sonst nichts. Agentenläufe stehen im Posteingang, Seitenfassungen in der Historie der Seite selbst, Leitlinien-Fassungen bei den Leitlinien. Lesezugriffe und Suchen, Anmeldungen, Änderungen an Schlüsseln und Rechten sowie Webhook-Zustellungen werden gar nicht aufgezeichnet. Ein Verursacher wird nur dort genannt, wo die Quelle einen führt.',
  'audit.empty.title': 'Noch nichts aufgezeichnet',
  'audit.empty.description': 'Sobald eine Wissensänderung entschieden wurde, steht sie hier.',
