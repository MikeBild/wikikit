...
import {
  ...
  staleSlugs,
  type ConceptDiff,
} from '@/pages/proposal-review.logic'
...
type ChangeDetail = Awaited<ReturnType<typeof wk.proposals.get>>
type StagedConcept = ChangeDetail['concepts'][number]
type LintReport = Awaited<ReturnType<typeof wk.proposals.lint>>
type LintFinding = LintReport['findings'][number]

export function ProposalReviewPage() {
  // `from` rather than `strict: false`: the route tree declares this detail address,
  ...
  const { id } = useParams({ from: '/decisions/proposals/$id' })
  const space = useSpace()

  const detail = useQuery({ queryKey: keys.proposal(id), queryFn: () => wk.proposals.get(id) })
  ...
  const lint = useQuery({ queryKey: keys.proposalLint(id), queryFn: () => wk.proposals.lint(id) })
...
function ChangeBody({
   ...
   *
   * The proposal key is also the prefix of its lint key, so one call refreshes
   * the diff and the lint together.
   ...
   */
  function refresh() {
    void queryClient.invalidateQueries({ queryKey: keys.proposal(id) })
    void queryClient.invalidateQueries({ queryKey: keys.proposals(space).slice(0, -1) })
    void queryClient.invalidateQueries({ queryKey: keys.attention(space).slice(0, -1) })
  }

  const approve = useMutation({
    mutationFn: async () => (await wk.proposals.approve(id)) as { concepts?: string[] } | null,
    onSuccess: (result) => {
      refresh()
      toast({
        tone: 'success',
  ...
  const reject = useMutation({
    mutationFn: () => wk.proposals.reject(id),
    onSuccess: () => {
      refresh()
      toast({
        tone: 'info',
  ...
  const requestChanges = useMutation({
    mutationFn: (revision: string) => wk.proposals.requestChanges(id, { note: revision }),
    onSuccess: () => {
      setNote('')
      refresh()
      toast({
  ...
  const split = useMutation({
    mutationFn: async (slugs: string[] | null) =>
      (await wk.proposals.split(id, slugs ? { concepts: slugs } : {})) as {
