...
export function SourcePage() {
  ...
  const title = source.data ? sourceLabel(source.data) : t('source.title')

  return (
    <Page title={title} description={source.data?.summary ?? t('source.description')}>
      {!id ? (
        <EmptyState
          title={t('source.missing.title')}
          description={t('source.missing.description')}
      ...
      <div className="flex flex-col gap-6">
        <Card>
          <CardHeader>
            ...
            <CardAction>
              <ContextHelp title={text('About the archive record')} testId="source-archive-help">
                <p>
                  What WikiKit knows about these bytes. The hash makes the same document ingested twice one source
                  rather than two.
                </p>
              </ContextHelp>
            </CardAction>
          ...
          <CardContent>
            <dl className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label={text('Retrieval language')}>
                <span data-testid="source-language">
                  {source.language
                    ? text(LANGUAGE_WORDS[source.language] ?? source.language)
                    : text("This wiki's language")}
                </span>
              </Field>
              ...
              <Field label={text('Content hash')}>
                <span className="flex flex-wrap items-center gap-2">
                  <Badge tone="unknown">{text('Source locked')}</Badge>
        ...
        {source.stream_id ? (
          <Card>
            <CardHeader>
              ...
              <CardAction>
                <ContextHelp title={text('About connector-managed sources')} testId="source-connector-help">
                  <p>
                    A stable upstream id lets each push archive a new version without forking the archive. Older
                    versions remain available to the claims that quote them.
                  </p>
                </ContextHelp>
              </CardAction>
            </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardAction>
              <ContextHelp title={text('About the archived document')} testId="source-document-help">
                <p>
                  {source.kind === 'url'
          ...
          <CardContent className="flex flex-col gap-3">
            <SegmentedControl
              label={text('How to read this document')}
              value={view}
              onChange={setView}
              data-testid="source-view"
              options={[
                { id: 'rendered', label: text('Rendered') },
                { id: 'verbatim', label: text('Verbatim') },
              ]}
            />
            {view === 'rendered' ? (
              /*
        ...
        <SourceReferences space={space} sourceId={source.id} />
      </div>
    </I18nText>
  )
}

function SourceReferences({ space, sourceId }: { space: string; sourceId: string }) {
  const { t } = useI18n()
  const references = useQuery({
    queryKey: keys.sourceReferences(space, sourceId),
    queryFn: () => wk.sources.references(space, sourceId, { limit: 100 }),
  })
  return (
    <Card data-testid="source-references">
      <CardHeader>
        <CardTitle>{t('source.references.title')}</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col gap-3">
        <p className="text-sm text-muted-foreground">{t('source.references.description')}</p>
        {references.isLoading ? <p className="text-sm text-muted-foreground">{t('common.loading')}…</p> : null}
        {references.isError ? <p className="text-sm text-destructive">{t('source.references.error')}</p> : null}
        {references.data?.items.length === 0 ? (
          <p className="rounded-lg border border-dashed p-3 text-sm text-muted-foreground">
            {t('source.references.empty')}
          </p>
        ) : null}
        {references.data?.items.length ? (
          <ul className="divide-y rounded-lg border">
            {references.data.items.map((reference, index) => (
              <li key={`${reference.kind}:${reference.id}`}>
                <Link
                  to={reference.href as never}
                  className="flex min-w-0 items-center justify-between gap-3 p-3 hover:bg-muted"
                  data-testid={`source-reference-${reference.kind}-${index + 1}`}
                >
                  <span className="truncate text-sm font-medium">{reference.label}</span>
                  <Badge tone={reference.kind === 'page' ? 'neutral' : 'warning'}>
                    {t(reference.kind === 'page' ? 'source.references.page' : 'source.references.proposal')}
                  </Badge>
                </Link>
              </li>
            ))}
