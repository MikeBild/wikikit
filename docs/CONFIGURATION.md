...
| `WIKIKIT_PUBLIC_URL`                | Public base URL — used in docs links and the MCP origin allowlist; trailing slash stripped        | `http://127.0.0.1:4060`                                            |
...
| `WIKIKIT_SKIP_DOTENV`               | `1` ignores an on-disk `.env` (still reads `.env.defaults`) — ops escape hatch for a stray `.env` | (unset)                                                            |
...
Only `ANTHROPIC_API_KEY` has no default: without it every LLM-free feature
(search, read, history, lint, export, import, review) works normally, while
ingest and query answer `503 llm_not_configured` with a hint.
