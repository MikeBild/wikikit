...
| `WIKIKIT_MODEL_SYNTHESIS`                        | Model for concept synthesis (one call per affected concept). Anthropic id by default — set it per provider, a foreign id fails the boot | `claude-sonnet-5`                                                  |
...
| `WIKIKIT_MODEL_ANSWER`                           | Model for grounded Q&A (`POST .../query`). Anthropic id by default — set it per provider, a foreign id fails the boot                   | `claude-sonnet-5`                                                  |
| `WIKIKIT_MODEL_PRICES`                           | JSON map of model ids to `{input,output,cache_read}` USD rates per one million measured tokens; malformed maps fail boot                | `{}` (no guessed prices)                                           |
| `WIKIKIT_ANSWER_TOKEN_BUDGET`                    | Maximum estimated evidence tokens packed into one grounded answer (1 000–200 000); skipped evidence is reported as truncated            | `36000`                                                            |
...
| `WIKIKIT_MODEL_EMBEDDING`                        | Embedding model — must produce 1536-dim vectors (the `wk_embeddings` pin); checked against `WIKIKIT_EMBEDDING_PROVIDER` at boot         | `text-embedding-3-small` (google: `gemini-embedding-001`)          |
...
model ids. Forgetting the three model variables no longer reaches a caller:
each is checked against `WIKIKIT_LLM_PROVIDER` at boot, and an id that
recognisably belongs to another provider refuses to start, naming the setting,
the model, both providers and the two ways to repair it:

```
WIKIKIT_MODEL_ANSWER=claude-sonnet-5 is a model id for anthropic, but
WIKIKIT_LLM_PROVIDER=openai — set WIKIKIT_MODEL_ANSWER to a model id for
openai, or set WIKIKIT_LLM_PROVIDER=anthropic
```

The check reads the id's shape (`claude-…` → anthropic, `gpt-…`/`o…` → openai,
`gemini-…` → google) and is deliberately one-sided: an id it does not recognise
boots. WikiKit runs as a binary for months, so a model released after your
build must never be what stops a service from starting — this catches the
obvious mismatch, it is not a catalogue of valid models. `/ready` reporting
`llm_configured: true` still only proves the key was supplied, so an unfamiliar
id is worth one real ingest and one real query after switching.
...
Anthropic serves no embedding model — and `WIKIKIT_MODEL_EMBEDDING` is held to
that switch by the same boot check, which matters more there than here: the
embedder is a background worker, so a foreign id would never fail a request an
operator is watching, only retry quietly in the log. The check runs only while
an embedding provider is selected; under `none` the value is carried and sent
nowhere.
