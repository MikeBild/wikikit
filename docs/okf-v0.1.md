<!--
  ...
  Source:    https://raw.githubusercontent.com/GoogleCloudPlatform/knowledge-catalog/main/okf/SPEC.md
  Retrieved: 2026-07-15
  Pinned as: OKF v0.1 (Draft)

  This file is the frozen reference that src/export/okf.ts and
  test/contract/okf-conformance.test.ts are written against (plan §9, §15.1:
  "Spec vendorn, Konformanz-Fixtures zuerst, alles OKF-Wissen in export/okf.ts").
  If upstream revises the spec, re-vendor it here FIRST, bump the version
  constant in src/export/okf.ts, and update the conformance fixtures — never
  adapt the exporter to an un-vendored spec change.

  Everything below this comment is the upstream document, verbatim.
-->
