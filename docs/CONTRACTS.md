...
| GET    | `/v1/spaces/{space}/search`                   | knowledge:read                     | `searchHandler`                | query `zSearchQuery`                                                   | 200 `zSearchResponse` (`kind:'concept'` hits carry `evidence` — same counts as the concept list, absent where it declines to measure; `claim`/`source_chunk` hits deliberately do not — §5.3) |
...
- `kind: 'concept'` — carries it, for every page the aggregate answers for. Two
  ...
  same two pages, so a hit and an index row never disagree about one slug.
