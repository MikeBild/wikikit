...
// wikikit_overview tool AND the agent briefing's pending_changes, so three
// surfaces cannot disagree about one backlog. Two GROUP BY statements total;
// empty input → empty Map, no query. pending_derived counts pending proposals
// whose EVERY cited source carries derived_from_output_id (provenance, never a
// verdict — mixed provenance does not count).
...
export interface OverviewRow {
  ...
  created_7d: number // proposals created in the last 7 days, any status
  pending_derived: number
  concepts: number // visible pages: current_revision_id set
}
// Composes reviewOverview over the spaces the KEY may see (the caller filters;
// this invents no visibility) into the wikikit.spaces-overview.v1 body minus
// the schema_version stamp, totals summed server-side.
...
| GET    | `/v1/stats/overview`                           | knowledge:read                      | `spacesOverviewHandler`        | —                                                                             | 200 `zSpacesOverviewResponse` (`schema_version: wikikit.spaces-overview.v1`; one row per space the key may see; totals server-side; a space-scoped key gets a one-row overview)                                                                                                                                                                                              |
...
| `wikikit_overview`        | knowledge:read    | `{}`                                                                                                                                                                                                                                                                                                                                                               | `zSpacesOverviewResponse` shape, identical to `GET /v1/stats/overview` — per visible space the review backlog with the age of its oldest change, `pending_derived` (provenance, never a verdict), 7-day activity, visible page count, totals server-side                                                                            | `true`   | `false`     | `true`     | `false`   |
