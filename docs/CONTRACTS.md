...
| PUT    | `/v1/identities/{provider}/{subject}`         | admin                              | `upsertIdentityHandler`        | params `zIdentityParams`; body `zUpsertIdentityRequest`   | 200/201 `zIdentityResponse` (409 `identity_revoked` without `restore:true`; 422 role XOR scopes / unknown provider / update would strip the stored ceiling to NULL) |
