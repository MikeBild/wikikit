...
export const TOOLS: McpToolDef[] = [
  ...
  },
  {
    ...
    description:
      ...
      'hits carry none, and neither does a page there is no measurement to give for. An absent summary is not a zero ' +
      'one. Where the page is a reference target — a row created to receive relations — the hit says so instead: ' +
      "not_measured:{reason:'reference_target'}, plus withheld_claims when that page nevertheless holds visible " +
      'claims, which is the count the measurement would have reported. A hit carrying neither field is a page that is ' +
      'no longer readable.',
    scope: 'knowledge:read',
